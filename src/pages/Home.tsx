...
const Home = () => {
  ...
  const onGetPlayers = () => {
    api.getPlayers(cookies["peareasy"]).then((players: SBC[]) => {
      ...
      const _players = players.map((player) => {
        ...
        return (
          <div
            className={"text-secondary bg-gray-600 rounded text-s p-1"}
            key={ctr}
          >
            {player}
          </div>
  ...
  const onSolveSBC = (sbc: string) => {
    setLoading(true);
    api
      .solveSBC(cookies["peareasy"], sbc)
      .then((solution: SBC[]) => {
        const players = solution.map((player) => (
          <div className={"text-primary-300 bg-gray-600 rounded p-1"}>
            {player}
          </div>
        ));
        setSolution(players);
      ...
      <div className="grid grid-cols-2 gap-y-4 gap-x-8">
        <div className="space-y-8">
          ...
          <div className="rounded p-6">
            <h1 className="text-secondary text-center text-3xl mt-8 mb-8">
              Your players
            </h1>
          </div>
          <div className="flex flex-wrap gap-2 mt-8">{players}</div>
        </div>
        <div className="space-y-8">
          ...
          <div className="flex flex-col gap-4 mt-8">
            {loading ? (
              ...
              <div className="flex flex-wrap gap-2">{solution}</div>
            )}
            {solution.length > 0 ? (
              <SecondaryButton
