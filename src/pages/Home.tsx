...
const Home = () => {
  ...
  const onSolveSBC = () => {
    ...
    setLoading(true);
    api
      .solveSBC(cookies, sbcs[selectedSBC])
  ...
  const getStartedView = (
    <div className="space-y-8">
      ...
      <p className="m-auto">
        Are you tired of spending time on solving squad building challenges
        (SBCs) in FIFA ultimate team?
      </p>
      <p className="m-auto">
        Then you have come to the right place! We've developed an artificial
        intelligence which searches for a cheap solution to an SBC with the
  ...
  const importPlayersView = (
    <div className="space-y-8">
      <h1 className="text-5xl font-bold m-auto">Import your players! ⚽</h1>
      ...
      <div>
        Click
        <a
          href="https://www.ea.com/fifa/ultimate-team/web-app/"
          className="no-underline m-auto"
          target="_blank"
        >
          {" "}
          here{" "}
    ...
    <div className="space-y-2">
      {sbcs.length > 0
        ? sbcs.map((sbc, index) => (
            <CardSBC
              title={sbc}
              onClick={() => setSelectedSBC(index)}
              selected={selectedSBC === index}
            />
          ))
        : null}
      <div className="absolute bottom-10 left-0 right-0">
  ...
  const progressBarClassNameParent = [
    "absolute bottom-0 h-2 w-full bg-primary-200 rounded overflow-hidden text-xs flex",
  ];
      ...
      <div role="alert">
        <div className="w-2/3 m-auto">
          ...
          <div className="border border-t-0 border-error-400 rounded-b bg-red-100 px-4 py-3 text-red-700 ">
            <p>
              It seems like your players weren't imported properly. Please, try
              again or see this Youtube tutorial
            </p>
          </div>
        </div>
        <div className="absolute bottom-10 left-0 right-0">
    ...
    <>
      <main className="w-2/6 mx-auto h-2/4 text-secondary text-center relative">
        <div className="mx-auto h-3/4 overflow-y-scroll">
          ...
          {steps === 3 ? loading ? <Spinner /> : solutionView : null}
        </div>
      </main>
      <WaveBackground />
    </>
